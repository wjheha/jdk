        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
    ...
    public void ensureCapacity(int minCapacity) {
        if (minCapacity > elementData.length  // 如果 minCapacity 大于数组的容量
            && !(elementData == DEFAULTCAPACITY_EMPTY_ELEMENTDATA  // 如果 elementData 是 DEFAULTCAPACITY_EMPTY_ELEMENTDATA 的时候，
                 && minCapacity <= DEFAULT_CAPACITY)) {  // 需要最低 minCapacity 容量大于 DEFAULT_CAPACITY ，因为实际上容量是 DEFAULT_CAPACITY 。
            // 数组修改次数加一
            modCount++;
            // 扩容
            grow(minCapacity);
    ...
    boolean equalsRange(List<?> other, int from, int to) {
        // 如果 to 大于 es 大小，说明发生改变，抛出 ConcurrentModificationException 异常
        final Object[] es = elementData;
        if (to > es.length) {
    ...
    /**
     * 批量移除 [fromIndex, toIndex) 的多个元素，注意不包括 toIndex 的元素噢。
     */
    /**
    ...
    /**
     * 求 elementData 数组和指定多个元素的交集。简单来说，恰好和 #removeAll(Collection<?> c) 相反，移除不在 c 中的元素。
     */
    /**
